...
Receive incoming data from an Atlas device over UDP and print out the incoming message
contents.
)EOF",
           argv[0]);
    return 0;
...
#if DEBUG_ON
    // Enable to debug udp server to client problems.
    printf("listener: received raw packet [%zu bytes] from %s\n", bytes_read,
           their_ip);
#endif
