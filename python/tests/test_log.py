...
def make_log_dir(base_dir, log_id, files, create_manifest=True):
    ...
    if create_manifest:
        make_manifest(log_dir=log_dir, data_filename=files[0], guid=log_id)
    return log_dir


def make_log_base_dir(tmpdir, log_id, files, create_manifest=True):
    """
    ...
    log_dir = make_log_dir(base_dir=nested, log_id=log_id, files=files, create_manifest=create_manifest)
...
class TestLocateLogDirectory:
    ...
    def test_dir_user_output(self, tmpdir):
        log_dir = make_log_dir(tmpdir, LOG_HASH, ['output/fusion_engine.p1log'], create_manifest=False)
    ...
    def test_dir_diag(self, tmpdir):
        log_dir = make_log_dir(tmpdir, LOG_HASH, ['output/diagnostics.p1log'], create_manifest=False)
        assert locate_log(log_dir, log_base_dir=str(tmpdir)) == \
            os.path.join(log_dir, 'output', 'diagnostics.p1log')

    def test_dir_input_p1log(self, tmpdir):
        log_dir = make_log_dir(tmpdir, LOG_HASH, ['input.p1log'], create_manifest=False)
        assert locate_log(log_dir, log_base_dir=str(tmpdir)) == \
            os.path.join(log_dir, 'input.p1log')

    def test_dir_manifest_data(self, tmpdir):
        log_dir = make_log_dir(tmpdir, LOG_HASH, ['test_file.bin'], create_manifest=True)
        assert locate_log(log_dir, log_base_dir=str(tmpdir)) == \
    ...
    def test_dir_no_candidates(self, tmpdir):
        log_dir = make_log_dir(tmpdir, LOG_HASH, ['irrelevant.txt'], create_manifest=False)
...
class TestLocateLogPatternMatch:
    ...
    def test_pattern_manifest_missing_data_file(self, tmpdir):
        log_base, log_dir = make_log_base_dir(tmpdir, LOG_HASH, [], create_manifest=False)
...
class TestLocateLogReturnModes:
    def _make_log(self, tmpdir, input_path='output/diagnostics.p1log', create_manifest=True):
        return make_log_base_dir(tmpdir, LOG_HASH, [input_path], create_manifest=create_manifest)

    def test_file_only(self, tmpdir):
        log_base, log_dir = self._make_log(tmpdir, create_manifest=False)
