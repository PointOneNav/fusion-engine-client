...
def find_p1log_file(input_path, return_output_dir=False, return_log_id=False, log_base_dir='/logs',
    ...
    @param load_original If `True`, load the `.p1log` file originally recorded with the Atlas log. Otherwise, load the
    ...
    # directory to the log directory (not the subdirectory containing the p1log file).
        ...
        candidate_files = [os.path.join(log_dir, 'output', 'fusion_engine.playback.p1log')
                           if not load_original else None,
                           os.path.join(log_dir, 'fusion_engine.p1log'),
