...
CANDIDATE_PLAYBACK_LOG_FILES = [
    # v- Output generated by log playback.
    'output/primary_output.playback.raw',
    # v- Fallback diagnostic log playback output if the primary output (i.e., typically an I/O interface configured by
    # the user like UART2) does not have any output configured/enabled.
    'output/diagnostic.playback.p1log',
    # Legacy path, maintained for backwards compatibility.
    'output/fusion_engine.playback.p1log',
...
def find_p1log_file(input_path, return_output_dir=False, return_log_id=False, log_base_dir=DEFAULT_LOG_BASE_DIR,
    ...
    if not load_original:
        candidate_files = CANDIDATE_PLAYBACK_LOG_FILES + CANDIDATE_LOG_FILES
    else:
...
def locate_log(input_path, log_base_dir=DEFAULT_LOG_BASE_DIR, return_output_dir=False, return_log_id=False,
    ...
    if log_file_path.endswith('.p1bin'):
        _logger.info("Found p1bin file '%s'. Extracting binary stream." % log_file_path)
        _, bin_files = dump_p1bin(input_path=log_file_path)
