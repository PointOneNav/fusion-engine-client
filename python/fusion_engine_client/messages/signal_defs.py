...
_GNSSSignalPartType: TypeAlias = SatelliteType | FrequencyBand | SignalName | ComponentType
...
_T = TypeVar('_T', SatelliteType, FrequencyBand, SignalName, ComponentType)
...
def _get_pretty_gnss_signal_type(name: str, sv_type: SatelliteType, signal_name: SignalName,
                                 component: ComponentType, omit_component_hint: bool = False) -> str:
    '''!
    ...
    component_str = ''
    if component != ComponentType.COMBINED:
        if signal_str.endswith('B/C'):
            end_char = 'B' if component == ComponentType.DATA else 'C'
...
def to_signal_val(sv_type: SatelliteType, freq_band: FrequencyBand, signal_name: SignalName,
                  component=ComponentType.COMBINED) -> int:
    '''!
...
def to_signal_type(sv_type: SatelliteType, freq_band: FrequencyBand, signal_name: SignalName,
                   component=ComponentType.COMBINED) -> 'GNSSSignalType':
    '''!
...
#   *SignalName (3b), ComponentType (2b) }
# ```
#
...
class GNSSSignalType(IntEnum):
    ...
    def get_component_type(self) -> ComponentType:
        return _get_signal_part(self, ComponentType, raise_on_unrecognized=False)
...
class SignalID:
    ...
    def get_component_type(self) -> ComponentType:
        return self.signal_type.get_component_type()
